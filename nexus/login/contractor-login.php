...
if(isset($_POST['contractor-submit'])){
  ...
  $select_user = "select * from customers where user_name='$username' AND password='$password'";
  if($conn->query($select_user) == TRUE){
    ...
    if($check == 1){
      ...
      echo "<script> window.open('login/contractor.php?user_name=$session_user', '_self')</script>";
    }
    else{?>
