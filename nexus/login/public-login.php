...
if(isset($_POST['public-submit'])){
  ...
  $select_user = "select * from public where user_name='$username' AND password='$password'";
  if($conn->query($select_user) == TRUE){
    ...
    if($check == 1){
      ...
      echo "<script> window.open('login/public.php?user_name=$session_user', '_self')</script>";
    }
    else{?>
